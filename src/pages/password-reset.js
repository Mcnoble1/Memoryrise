...
const PasswordReset = () => {
  ...
  const ResetPassword = async (values) => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/user/create/password-reset-link`,
      {
        method: "PUT",
        body: JSON.stringify({
          ...values,
    ...
    const link = host + "/password-change/" + res;

    const message = `
    <div>
      <p>Click this <a href="${link}">link</a> to reset you password. If you didn't request password reset, ignore this email.</p>
    </div>
    `;
    sendEmail(message, values.email);
  };
  ...
  const sendEmail = async (message, email) => {
    ...
    setPResult("Password reset link sent to your email successfully!");
    setIsError(false);
  };
            ...
            <Form className={classes.form}>
              <Container>
                <Grid container direction="column" spacing={3} sx={{ mt: "10px" }}>
                  ...
                  <Grid item md={12}>
                    <Typography>Please provide your email address</Typography>
                  </Grid>
